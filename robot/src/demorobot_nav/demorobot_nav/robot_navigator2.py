...
class LaunchThread(threading.Thread):
    ...
    def run(self):
        ...
        # command = ["ros2", "launch", self.package, self.launch_file, 'namespace:=' + self.namespace]
        # if self.namespace:
        #     command.append('namespace:=' + self.namespace)
...
class RunThread(threading.Thread):
    ...
    def run(self):
        ...
        self.process = subprocess.Popen(["ros2", "run", self.package, self.run_file])
        self.process.wait()
...
class RobotNavigator(Node):
    def __init__(self):
        ...
        self.nav_cnt = 0
        self.pub_navigation_result = self.create_publisher(Int8, self.NAMESPACE + '/navigation_result', amcl_pose_qos)
...
def main(args=None):
    ...
    # wanderer_manager = RunThread("demorobot_wander", "manager")
    # wanderer_manager.start()
