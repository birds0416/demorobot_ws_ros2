...
class Subscriber(Node):
    ...
    def wander_stop_callback(self, msg):
        self.isWanderStop.data = msg.data
...
def stop_robot(subscriber, publisher, command):
    command = reset_commands(command)
    rclpy.spin_once(subscriber)
...
class WandererServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        ''' wander 정지 '''
        feedback_msg = Wander.Feedback()
        feedback_msg.stop_wandering = False
    ...
    def handle_cancel(self, goal_handle):
        ...
        self.get_logger().info("Goal Cancelation Requested")
        return CancelResponse.ACCEPT
...
def main(args=None):
    ...
    wanderer_server.stop_wandering_callback()
    rclpy.shutdown()
